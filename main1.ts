...
async function getCasts(fid: number, limit: number, cursor?: string): Promise<FetchCastsResult> {
    try {
        const result = await fetchData(
            ...
            cursor ? '/v2/casts?fid=%d&limit=%d&cursor=%s' : '/v2/casts?fid=%d&limit=%d',
            cursor ? [fid, limit, cursor] : [fid, limit],
...
async function getHiddenReplies(focusedCastHash: string, limit: number, cursor?: string): Promise<Reply[]> {
    try {
        const result = await fetchData(
            'https://client.warpcast.com',
            '/v1/user-thread-hidden-replies?focusedCastHash=%s&limit=%d',
            [focusedCastHash, limit],
...
checkCastsAndHiddenRepliesForMultipleFids([230238, 422233, 345765]).then(() => {
